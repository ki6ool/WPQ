...
// 	$photo = $file_dir . basename($url);
	$photo = $file_dir . strstr(basename($url), "?", true);//add
 ...
 * add
 */
function _insert_attachment($media, $title, $caption, $id) {
	require_once(ABSPATH . "wp-admin" . '/includes/image.php');
	require_once(ABSPATH . "wp-admin" . '/includes/file.php');
	require_once(ABSPATH . "wp-admin" . '/includes/media.php');
	$filetype = wp_check_filetype(basename($media), null);
	...
	$attach_id = wp_insert_attachment($attachment, $media, 0);
	if ( !is_wp_error($attach_id) ) {
		wp_update_attachment_metadata($attach_id, wp_generate_attachment_metadata($attach_id, $media));
		return $attach_id;
	}
	return false;
}
